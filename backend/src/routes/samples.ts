 ...
 * Requires authenticated user (any role that can submit detection records).
 */
router.post('/:id/compare', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (!latestVersions?.length) { res.json({ message: 'no versions' }); return }
